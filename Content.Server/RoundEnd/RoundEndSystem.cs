...
namespace Content.Server.RoundEnd
{
    ...
    public sealed partial class RoundEndSystem : EntitySystem
    {
        ...
        /// <summary>
        /// Read-only telemetry: remaining seconds until the shuttle countdown completes.
        /// Negative values indicate the countdown is overdue.
        /// Null when the countdown is not active.
        /// </summary>
        public double? CountdownRemainingSeconds =>
            IsRoundEndRequested()
                ? ShuttleTimeLeft?.TotalSeconds
                : null;
        ...
        /// <summary>
        /// Read-only telemetry: whether an auto-call threshold elapsed and a request was attempted.
        /// RequestRoundEnd can return without accepting the request, so this does not imply success.
        /// </summary>
        public bool AutoCallAttempted => _autoCalledBefore;

        /// <summary>
        /// Read-only telemetry: the currently selected auto-call delay in minutes (0 means disabled).
        /// Reflects the extension time after the first auto-call, the initial time before it.
        /// </summary>
        public int SelectedAutoCallDelayMinutes =>
            _autoCalledBefore
                ? _cfg.GetCVar(CCVars.EmergencyShuttleAutoCallExtensionTime)
                : _cfg.GetCVar(CCVars.EmergencyShuttleAutoCallTime);

        /// <summary>
        /// Read-only telemetry: elapsed minutes since the auto-call timer started.
        /// Null when auto-call is disabled (SelectedAutoCallDelayMinutes is 0).
        /// </summary>
        public double? AutoCallElapsedMinutes =>
            SelectedAutoCallDelayMinutes == 0
                ? null
                : (_gameTiming.CurTime - AutoCallStartTime).TotalMinutes;
