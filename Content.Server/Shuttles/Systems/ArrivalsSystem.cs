...
public sealed partial class ArrivalsSystem : EntitySystem
{
    ...
    public void HandlePlayerSpawning(PlayerSpawningEvent ev)
    {
        ...
        if (!TryComp<StationArrivalsComponent>(ev.Station, out var stationArrivals))
            return;

        // SOLREIGN fix (loadgate x arrivals stranding): the GLOBAL arrivals source/terminal map can be
        // fine (SetupArrivalsStation succeeded) even when THIS station's own arrivals shuttle failed to
        // load or dock — SetupShuttle catches and logs that failure so one station's shuttle problem
        // doesn't abort the whole round, leaving StationArrivalsComponent.Shuttle Deleted/invalid for
        // that station. If we still spawn a late joiner at the arrivals terminal in that case, nothing
        // will ever be scheduled to ferry them to this station (the periodic Update() loop only moves
        // ArrivalsShuttleComponent shuttles that exist) — they end up permanently off-station with no way
        // in. Require a usable shuttle for THIS station before arrivals-spawning; otherwise fall through
        // to the normal on-station spawn (SpawnPointSystem, subscribed immediately after us).
        if (Deleted(stationArrivals.Shuttle) || !HasComp<ArrivalsShuttleComponent>(stationArrivals.Shuttle))
            return;
    ...
    private void SetupArrivalsStation()
    {
        ...
        // Arrivals is an OPTIONAL feature: if the arrivals terminal grid or shuttle fails to load
        // (missing/broken map file, a map referencing a prototype this fork removed, a docking failure,
        // etc.) we must NOT let the exception propagate out of the RoundStartingEvent handler. That event
        // is broadcast inside GameTicker.StartRound's try (GameTicker.RoundFlow.cs), and RobustToolbox's
        // broadcast dispatch has no per-handler try/catch, so a throw here aborts the whole round and
        // (under EXCEPTION_TOLERANCE) triggers a restart loop. Instead we log loudly and run the round
        // WITHOUT arrivals; HandlePlayerSpawning falls through to the normal on-station spawn when no
        // arrivals grid exists, which is the correct upstream fallback.
        try
        {
            ...
            {
                Log.Error(
                    $"Failed to load arrivals terminal grid '{path}'. Running the round WITHOUT arrivals; " +
                    "players will spawn directly on-station.");
                CleanupFailedArrivalsMap(mapId);
                return;
            }
        ...
        {
            Log.Error(
                $"Exception while setting up the arrivals station from '{path}'. Running the round WITHOUT " +
                $"arrivals; players will spawn directly on-station.\n{e}");
            // Tear down the partially-created arrivals map/grids so no half-initialized arrivals state
            // lingers and confuses spawning. TryGetArrivals will then return false and players spawn normally.
            CleanupFailedArrivalsMap(mapId);
        }
    }

    /// <summary>
    /// Cleans up a partially-created arrivals map after a setup failure so the round can proceed without
    /// arrivals. Removes the arrivals map plus any arrivals source/shuttle grids that were created before
    /// the failure, leaving the game in a clean "no arrivals" state.
    /// </summary>
    private void CleanupFailedArrivalsMap(MapId mapId)
    {
        try
        {
        ...
        {
            // Cleanup itself must never take down the round.
            Log.Error($"Exception while cleaning up a failed arrivals map.\n{e}");
        }
    }
    ...
    private void SetupShuttle(EntityUid uid, StationArrivalsComponent component)
    {
        ...
        // Loading/docking the arrivals shuttle can throw (missing/broken shuttle grid, a shuttle grid
        // lacking a ShuttleComponent, or a station map with no valid arrivals dock so FTLToDock fails).
        // This is called from the round-start path as well as StationPostInit and the admin command, all of
        // which run inside try-guarded round flow. A throw here would abort/restart the round, so a single
        // station's shuttle failure is logged and that station simply gets no arrivals shuttle.
        try
        {
            if (TryGetArrivals(out var arrivals) &&
        ...
        {
            Log.Error(
                $"Failed to set up the arrivals shuttle '{component.ShuttlePath}' for station " +
                $"{ToPrettyString(uid)}. This station will run without an arrivals shuttle.\n{e}");
        }

        // Don't start the arrivals shuttle immediately docked so power has a time to stabilise?
        // (Always runs so the dummy map is cleaned up even if the shuttle load/dock above failed.)
