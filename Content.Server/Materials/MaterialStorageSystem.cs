...
public sealed partial class MaterialStorageSystem : SharedMaterialStorageSystem
{
    ...
    public override void Initialize()
    {
        ...
        // EntityTerminatingEvent (NOT ComponentShutdown) deliberately: it fires once, up front,
        // before ANY of the entity's components start shutting down (see
        // EntityManager.RecursiveFlagEntityTermination), so MaterialStorageComponent is guaranteed
        // to still be fully intact here regardless of the (unspecified/component-set-dependent)
        // order components get torn down in. A ComponentShutdown-based check here would be a race:
        // MaterialStorageComponent and StationDataComponent are both "arbitrary order" tier in
        // EntityManager.InSafeOrder, so whichever of the two happens to be removed first could
        // leave the other's shutdown handler unable to see it - silently reintroducing exactly the
        // "vanishes on deletion" bug this exists to close, just nondeterministically.
        SubscribeLocalEvent<StationDataComponent, EntityTerminatingEvent>(OnStationTerminating);
    ...
    /// <summary>
    /// A station's shared material pool (see SharedOreSiloSystem) must never silently vanish
    /// along with the station entity itself - e.g. an admin deleting the station mid-round, or any
    /// other path that terminates the entity outside of the normal deconstruction flow above
    /// (which is deliberately disabled for the pool - see LockDownPool). Dump its contents onto
    /// the largest surviving member grid instead of letting them disappear into the station
    /// entity's nullspace coordinates. If no grid survives there is nothing sane to do, but this
    /// still must never duplicate the materials - EntityTerminatingEvent only ever fires once.
    /// </summary>
    private void OnStationTerminating(EntityUid uid, StationDataComponent stationData, ref EntityTerminatingEvent args)
    {
        if (!TryComp<MaterialStorageComponent>(uid, out var component) || component.Storage.Count == 0)
            return;

        if (_station.GetLargestGrid((uid, stationData)) is not { } grid)
            return;

        var coordinates = new EntityCoordinates(grid, default);

        foreach (var (material, amount) in component.Storage)
        {
            SpawnMultipleFromMaterial(amount, material, coordinates);
        }
    }
    ...
    private void OnEjectMessage(EjectMaterialMessage msg, EntitySessionEventArgs args)
    {
        ...
        // Defense-in-depth: EjectMaterialMessage can name an arbitrary entity UID, and
        // CanInteract() above only checks the player's own action blockers (consciousness,
        // restraints, etc) - not that the player is anywhere near, or has line of sight to, the
        // target. Require the same in-range/unobstructed check any other direct interaction with
        // the target would need. Globally-networked entities (e.g. a station's material pool) are
        // additionally locked out entirely via CanEjectStoredMaterials below.
