...
namespace Content.Server.Database
{
    public interface IServerDbManager
    {
        ...
        /// <summary>
        /// Associates a player with a round. Prefer the identity-carrying overload below whenever the
        /// connecting session's real username/address/HWID are available (i.e. any caller reacting to a
        /// live <see cref="Robust.Shared.Player.ICommonSession"/>) -- this array-only overload has no
        /// identity to fall back on, so if a given player's row does not exist yet, it does NOT create one
        /// (a blank placeholder row is exactly the identity-corruption problem described on the
        /// identity-carrying overload below); that player's round association is logged and skipped
        /// instead. It's kept for callers that only ever deal with players who are already known to have a
        /// row (e.g. re-associating existing players).
        /// </summary>
        Task AddRoundPlayers(int id, params Guid[] playerIds);

        /// <summary>
        /// Associates a player with a round, using <paramref name="userName"/>/<paramref name="address"/>/
        /// <paramref name="hwId"/> as the authoritative identity to write if (and only if) this player does
        /// not have a Player row yet. This is the mid-round-connect path: a brand-new player's row is
        /// normally created by <see cref="UpdatePlayerRecordAsync"/> during the connection handshake, but
        /// that is not a guarantee this method can rely on (guests never get a row written that way, and a
        /// brand-new session can reach <see cref="Robust.Shared.Enums.SessionStatus.Connected"/> the instant
        /// it connects mid-round, before the handshake's own write lands). Supplying the real identity here
        /// -- rather than writing a blank/sentinel placeholder row -- means ban-audit and "is this a new
        /// player" consumers (<c>BanMatcher</c>, <c>PlayerLocator</c>, <c>ServerApi</c>) never see corrupted
        /// data: whichever of this call or <see cref="UpdatePlayerRecordAsync"/> wins the race to create the
        /// row, the row ends up holding the same real data either way.
        /// </summary>
        Task AddRoundPlayers(int id, NetUserId playerId, string userName, IPAddress address, ImmutableTypedHwid? hwId);
    ...
    public sealed partial class ServerDbManager : IServerDbManager
    {
        ...
        public Task AddRoundPlayers(int id, NetUserId playerId, string userName, IPAddress address, ImmutableTypedHwid? hwId)
        {
            DbWriteOpsMetric.Inc();
            return RunDbCommand(() => _db.AddRoundPlayers(id, playerId, userName, address, hwId));
        }
