...
""");
        ...
        // Model.cs declares two separate unique enforcements on Player.UserId: an EF "alternate key"
        // (backing the AdminLogPlayer/AdminNote FK relationships that target it), migrated in as the named
        // constraint "ak_player_user_id", and a plain unique index, migrated in as "IX_player_user_id". A
        // duplicate-UserId insert can be reported against either name depending on which one Postgres
        // happens to check first, so both are recognised here.
        protected override bool IsPlayerUserIdConflict(DbUpdateException ex)
        {
            return ex.InnerException is Npgsql.PostgresException
            {
                ...
                && (pg.ConstraintName == "ak_player_user_id" || pg.ConstraintName == "IX_player_user_id");
        }
