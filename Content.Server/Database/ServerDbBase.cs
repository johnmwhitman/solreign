...
namespace Content.Server.Database
{
    public abstract class ServerDbBase
    {
            ...
            ImmutableTypedHwid? hwId)
        {
            ...
            if (!inserting)
            {
                // The common case: the row already existed (normally created by this same method during
                // an earlier connection), so this is a plain update. No unique-constraint insert race is
                // possible on this path.
                await db.DbContext.SaveChangesAsync();
                return;
            }

            // Test-only hook: see the doc comment on TestBeforeUpdatePlayerRecordInsert.
            if (TestBeforeUpdatePlayerRecordInsert != null)
                await TestBeforeUpdatePlayerRecordInsert();

            try
            {
                await db.DbContext.SaveChangesAsync();
            }
            catch (DbUpdateException ex) when (IsPlayerUserIdConflict(ex))
            {
                // Reverse of the race AddRoundPlayers/EnsurePlayerRow handles below: AddRoundPlayers (or a
                // second concurrent UpdatePlayerRecord call for the same brand-new user -- e.g. a very fast
                // reconnect) won the insert for this UserId between our SingleOrDefaultAsync check above and
                // our SaveChangesAsync here. Our own insert is now a duplicate. Drop our failed insert and
                // reconcile against the row that won instead. Do not treat this as fatal -- let
                // lock/connection/other-constraint errors (anything IsPlayerUserIdConflict doesn't
                // specifically recognise) continue to propagate with their real diagnosis.
                db.DbContext.ChangeTracker.Clear();

                record = await db.DbContext.Player.SingleAsync(p => p.UserId == userId.UserId);

                // Ordering-safe recovery: only overwrite the committed row's identity/LastSeen* fields if
                // THIS call is actually newer than what's already committed. `now` was captured at the top
                // of this method, before the race was resolved, so an older losing call can reach this catch
                // block *after* a newer call already won and committed -- unconditionally overwriting here
                // would let the older call stomp the newer winner's username/IP/HWID and walk LastSeenTime
                // backward, potentially below the winner's FirstSeenTime. FirstSeenTime is never touched on
                // this path -- it belongs to whichever write actually won the physical INSERT -- so gating
                // the overwrite on `now > record.LastSeenTime` is sufficient to guarantee
                // FirstSeenTime <= LastSeenTime always holds: LastSeenTime can only move forward, and it
                // started >= FirstSeenTime the moment the row was inserted.
                if (now > record.LastSeenTime)
                {
                    record.LastSeenTime = now;
                    record.LastSeenAddress = address;
                    record.LastSeenUserName = userName;
        ...
        /// <summary>
        /// Ensures a Player row exists for <paramref name="playerId"/>, returning its row id. If no row
        /// exists yet, one is created using the supplied identity fields -- this is the mid-round-connect
        /// fallback path (see the doc comment on <see cref="IServerDbManager"/>'s
        /// identity-carrying overload): a brand-new player's row is normally created by
        /// <see cref="UpdatePlayerRecord"/> during the connection handshake, but that ordering is a property
        /// of one specific caller, not a guarantee this method can rely on, so it tolerates -- and recovers
        /// from -- losing an insert race against a concurrent <see cref="UpdatePlayerRecord"/> call for the
        /// same brand-new user.
        /// </summary>
        /// <remarks>
        /// If a row already exists, its fields are left untouched -- this method never overwrites existing
        /// identity data, authoritative or not. Reconciling/refreshing identity fields is
        /// <see cref="UpdatePlayerRecord"/>'s job alone.
        /// </remarks>
        private async Task<int> EnsurePlayerRow(
            DbGuard db,
            Guid playerId,
            string userName,
            IPAddress address,
            ImmutableTypedHwid? hwId)
        {
            var existingId = await db.DbContext.Player
            ...
            // Test-only hook: see the doc comment on TestBeforeAddRoundPlayersInsert.
            if (TestBeforeAddRoundPlayersInsert != null)
                await TestBeforeAddRoundPlayersInsert();
            ...
            {
                // Lost a race with a concurrent insert for the same user -- e.g. UpdatePlayerRecord's
                // connection-handshake write completing at the same moment, or another concurrent
                // AddRoundPlayers call for the same brand-new user. Drop our failed insert and adopt
                // whichever row won instead: it is either the exact same data we tried to write (both sides
                // of this race read from the same live session) or the connection handshake's authoritative
                // write, so there is nothing to reconcile here -- unlike UpdatePlayerRecord, this method never
                // overwrites an existing row's fields. Let lock/connection/other-constraint errors (anything
                // IsPlayerUserIdConflict doesn't specifically recognise) continue to propagate with their real
                // diagnosis instead of being silently swallowed here.
                db.DbContext.ChangeTracker.Clear();
            }
        ...
        /// <summary>
        /// Test-only synchronization hook, awaited (if set) immediately after the "row does not exist" check
        /// and before the row-creating INSERT is saved, in <see cref="EnsurePlayerRow"/> (used by the
        /// identity-carrying <see cref="AddRoundPlayers(int, NetUserId, string, IPAddress,
        /// ImmutableTypedHwid?)"/> overload -- the array overload no longer calls <see cref="EnsurePlayerRow"/>
        /// at all, since it has no identity to give a newly-created row and must not manufacture a blank
        /// one). Lets a test deterministically manufacture the actual mid-round-connect DB race instead of
        /// relying on wall-clock timing -- which the standard integration-test harness cannot produce anyway,
        /// since in-memory SQLite tests pin DB concurrency to a single connection at a time (see
        /// ServerDbSqlite's <c>inMemory ? 1 : ...</c> concurrency clamp), so two writers can never naturally
        /// interleave their reads under that harness. Always null outside of
        /// SolreignMidRoundConnectDbRaceTest. Internal: only ever set by Content.IntegrationTests (see
        /// Content.Server/AssemblyInfo.cs InternalsVisibleTo("Content.IntegrationTests")).
        /// </summary>
        internal static Func<Task>? TestBeforeAddRoundPlayersInsert;

        /// <summary>
        /// Same as <see cref="TestBeforeAddRoundPlayersInsert"/>, but for <see cref="UpdatePlayerRecord"/>'s
        /// own insert attempt.
        /// </summary>
        internal static Func<Task>? TestBeforeUpdatePlayerRecordInsert;

        /// <summary>
        /// True if <paramref name="ex"/> is specifically a violation of the Player.UserId unique constraint
        /// (see Content.Server.Database/Model.cs's <c>HasIndex(p =&gt; p.UserId).IsUnique()</c>) -- i.e. a lost
        /// insert race between <see cref="UpdatePlayerRecord"/> and <see cref="EnsurePlayerRow"/> for the same
        /// brand-new user. Any other <see cref="DbUpdateException"/> (lock contention, connection failure, an
        /// unrelated constraint) must propagate with its real diagnosis instead of being treated as this one
        /// specific, recoverable, expected race.
        /// </summary>
        protected abstract bool IsPlayerUserIdConflict(DbUpdateException ex);
        ...
        public async Task AddRoundPlayers(int id, Guid[] playerIds)
        {
            ...
            // A player's row is normally created by UpdatePlayerRecordAsync, which the connection
            // handshake awaits before a session ever reaches SessionStatus.Connected. But that ordering
            // is a property of one specific caller, not a guarantee this method can rely on: guest
            // connections never get a row written via that path at all, and a brand-new session can reach
            // SessionStatus.Connected the instant it connects mid-round, before the handshake's own write
            // lands.
            //
            // This overload has no real identity for a missing player to fall back on. It previously
            // papered over that with a blank ("", 0.0.0.0) placeholder row via EnsurePlayerRow -- but that
            // is itself a footgun: a blank row is a permanent, authoritative-looking identity that silently
            // poisons ban-audit / "is this a new player" consumers (BanMatcher, PlayerLocator, ServerApi)
            // for as long as it's never overwritten (e.g. a guest connection that never calls
            // UpdatePlayerRecordAsync at all). No production caller has real identity unavailable to it --
            // every caller with a live session uses the identity-carrying overload below instead -- so
            // rather than manufacture a fake identity here, a missing row is logged and that player's
            // round association is skipped. This can only silently under-report round-player history for a
            // caller that both lacks identity AND reaches a player with no existing row; it can never
            // corrupt the Player table with a blank row.
            var missingPlayers = playerIds.Where(playerId => !players.ContainsKey(playerId)).ToArray();
            if (missingPlayers.Length > 0)
            {
                _opsLog.Warning(
                    $"AddRoundPlayers(round {id}): {missingPlayers.Length} player(s) have no existing " +
                    "Player row and this overload has no identity to create one with -- skipping their " +
                    $"round association rather than writing a blank placeholder row: " +
                    string.Join(", ", missingPlayers));
            }

            foreach (var player in playerIds)
            {
                if (!players.TryGetValue(player, out var playerRowId))
                    continue;

                await db.DbContext.Database.ExecuteSqlAsync($"""
INSERT INTO player_round (players_id, rounds_id) VALUES ({playerRowId}, {id}) ON CONFLICT DO NOTHING
""");
            }
        ...
        /// <summary>
        /// See <see cref="IServerDbManager"/>'s identity-carrying <c>AddRoundPlayers</c> overload for why
        /// this exists: it lets a brand-new player's row, if <see cref="EnsurePlayerRow"/> must create one
        /// here, hold their real identity instead of a blank placeholder.
        /// </summary>
        public async Task AddRoundPlayers(int id, NetUserId playerId, string userName, IPAddress address, ImmutableTypedHwid? hwId)
        {
            await using var db = await GetDb();

            var playerRowId = await EnsurePlayerRow(db, playerId.UserId, userName, address, hwId);
