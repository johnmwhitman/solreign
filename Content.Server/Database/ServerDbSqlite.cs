...
namespace Content.Server.Database
{
    ...
    public sealed class ServerDbSqlite : ServerDbBase
    {
        ...
        // SQLite doesn't surface the name of the specific index/constraint it hit (unlike Postgres) --
        // Microsoft.Data.Sqlite's SqliteException only reports the affected table.column in its message.
        // Model.cs's Player.UserId has two separate unique enforcements (the "ak_player_user_id" alternate
        // key and the "IX_player_user_id" index; see the Postgres implementation of this method), but both
        // target the exact same single column, so matching on "player.user_id" here is unambiguous.
        protected override bool IsPlayerUserIdConflict(DbUpdateException ex)
        {
            return ex.InnerException is Microsoft.Data.Sqlite.SqliteException { SqliteErrorCode: 19 } sqlite // SQLITE_CONSTRAINT
                && sqlite.Message.Contains("player.user_id", StringComparison.OrdinalIgnoreCase);
        }
