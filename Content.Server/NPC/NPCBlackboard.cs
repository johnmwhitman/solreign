...
public sealed partial class NPCBlackboard : IEnumerable<KeyValuePair<string, object>>
{
    ...
    private bool TryGetEntityDefault(string key, [NotNullWhen(true)] out object? value, IEntityManager entManager)
    {
        ...
        // Solreign: this used to eagerly resolve HandsSystem here, unconditionally, for every single
        // key lookup -- including keys like Access/CanMove/OwnerCoordinates that never touch hands at
        // all. That meant any blackboard lookup, for any reason, paid for (and could fail on) a
        // HandsSystem resolution it didn't need. It's part of why a leaked/stale-teardown HTN plan-job
        // race (HANDOFF §10) kept "rotating" its crash site across unrelated preconditions -- whichever
        // key happened to be requested during the race window tripped this same shared call. Resolve
        // lazily, only in the switch cases that actually need it.
        HandsSystem? handSys = null;
        ...
        switch (key)
        {
            ...
            case ActiveHand:
            {
                handSys ??= entManager.System<HandsSystem>();
