...
public sealed partial class HTNSystem : EntitySystem
{
    ...
    // Solreign (HANDOFF §10 v2): NOT readonly. See DiscardPlanQueue below -- this gets
    // swapped out (not just drained) on system shutdown and round restart so a leaked
    // HTNPlanJob can never resume against a torn-down round.
    private JobQueue _planQueue = new(0.004);
    ...
    public override void Initialize()
    {
        ...
        // Solreign (HANDOFF §10 v2): the real leak site. HTNComponent.ComponentShutdown
        // (OnHTNShutdown, below) fires per-entity, but GameTicker's round restart deletes
        // NPCs via EntityManager.FlushEntities() -- fine on its own -- while the integration
        // test pool's own recycle path (RobustToolbox Pool/TestPair, engine-side, off-limits)
        // only awaits tick-idle before handing the SAME server/EntityManager/HTNSystem
        // instance to the next test. Cancelling a job's token is cooperative-only: the
        // CancellationTokenSource flag flips immediately, but the HTNPlanJob object itself
        // keeps sitting in _planQueue's internal pending/waiting lists until the JobQueue
        // happens to call Run() on it again -- there is no guarantee that happens before
        // "tick idle" is declared and the next test's ticks start flowing through the same
        // queue. A job that resumes on one of those later ticks does see
        // Cancellation.IsCancellationRequested and bails (HTNPlanJob.cs), IF it re-checks
        // at that exact resume point -- but the surest fix is to never let the queue itself
        // survive past a round boundary: throw the whole JobQueue away on round restart, not
        // just its jobs' tokens, so there's no queue left for a leaked job to be re-dequeued
    ...
    /// <summary>
    /// Cancels every in-flight plan job's token (belt and braces for anything currently
    /// mid-Run()) AND discards the JobQueue itself, replacing it with a fresh empty one.
    /// Cancelling a token alone only asks a job to stop next time it's resumed; it does not
    /// remove the job object from the queue, so a job that's slow to notice (or, per
    /// HANDOFF §10, one whose owning round/EntityManager gets recycled by the integration
    /// test pool before the queue ever re-dequeues it) can still be Run() again later against
    /// torn-down state. Dropping the queue object entirely is what actually guarantees that:
    /// no queue means Run() can never be called on any job that was in it, no matter how many
    /// ticks or tests later the JobQueue would otherwise have gotten around to it.
    /// </summary>
    private void DiscardPlanQueue()
    {
