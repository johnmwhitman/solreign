...
public sealed class HTNPlanJob : Job<HTNPlan>
{
    ...
    protected override async Task<HTNPlan?> Process()
    {
        ...
        {
            // Solreign (HANDOFF §10 v2): corrected mechanism -- an earlier pass here assumed
            // operator continuations could resume on arbitrary thread-pool threads. They can't:
            // RobustToolbox's TaskManager/RobustSynchronizationContext always funnels awaits back
            // onto this server's own single instance thread (ProcessPendingTasks, ahead of every
            // tick). The real gap is queue-lifetime, not threading: this job object can keep
            // sitting inside HTNSystem's JobQueue (Waiting/Pending) across MANY ticks. Cancelling
            // PlanningToken on entity/system shutdown only flips a flag -- it doesn't dequeue this
            // job -- so if the round restarts (or, in integration tests, the pooled server gets
            // handed to the next test) before the queue happens to Run() this job again, it can
            // resume here against a blackboard/EntityManager the new round/test doesn't own,
            // NRE'ing in whichever precondition runs next (rotating victim). HTNSystem now also
            // discards the whole JobQueue on round restart/shutdown (see HTNSystem.DiscardPlanQueue)
            // so a leaked job normally has no queue left to be re-dequeued from at all. This check
            // stays as defense in depth for the residual window (Shutdown() cancels tokens on the
            // same tick it discards the queue; a job already mid-Run() that same tick would still
            // see the cancelled token here before touching the blackboard again).
    ...
    private async Task<bool> PrimitiveConditionMet(HTNPrimitiveTask primitive, NPCBlackboard blackboard, List<Dictionary<string, object>?> appliedStates)
    {
        ...
        // Solreign (HANDOFF §10 v2): re-check the instant we resume from Operator.Plan (e.g.
        // MoveToOperator awaiting PathfindingSystem.GetPath), before this continuation gets
        // another chance to mutate the blackboard or fall back into the main loop to evaluate
        // more tasks. See the top-of-loop comment in Process() for why this job can still be
        // resuming at all after its round/EntityManager is gone (queue-lifetime, not threading).
        if (Cancellation.IsCancellationRequested)
            return false;
