...
public sealed partial class ServerApi : IPostInjectInit
{
    ...
    private async Task InfoHandler(IStatusHandlerContext context, Actor actor)
    {
        ...
        */
        ...
        var info = await RunOnMainThread<InfoResponse>(() =>
        {
            ...
            var roundEndSystem = _entitySystemManager.GetEntitySystem<RoundEndSystem>();
            var shuttleSystem = _entitySystemManager.GetEntitySystem<EmergencyShuttleSystem>();

            var roundHealth = new RoundHealthSnapshot
            {
                RunLevel = ticker.RunLevel.ToString(),
                AutoCallDelayMinutes = roundEndSystem.SelectedAutoCallDelayMinutes == 0
                    ? null
                    : roundEndSystem.SelectedAutoCallDelayMinutes,
                AutoCallElapsedMinutes = roundEndSystem.AutoCallElapsedMinutes,
                AutoCallAttempted = roundEndSystem.AutoCallAttempted,
                RoundEndRequested = roundEndSystem.IsRoundEndRequested(),
                CountdownRemainingSeconds = roundEndSystem.CountdownRemainingSeconds,
                EmergencyShuttleArrived = shuttleSystem.EmergencyShuttleArrived,
                ShuttlesLeft = shuttleSystem.ShuttlesLeft,
                RoundEndCompletionTokenArmed = shuttleSystem.RoundEndCompletionTokenArmed
            };
    ...
    private sealed class InfoResponse
    {
        ...
        public required RoundHealthSnapshot RoundHealth { get; init; }
...
/// <summary>
/// Read-only round-health telemetry snapshot exposed via /admin/info.
/// Contains the minimum stable fields needed to distinguish:
/// auto-call timer non-transition, shuttle-arrival completion stall, and explicit delayroundend.
/// </summary>
public sealed class RoundHealthSnapshot
{
    /// <summary>
    /// Current game run level: "PreRoundLobby", "InRound", or "PostRound".
    /// </summary>
    public required string RunLevel { get; init; }

    /// <summary>
    /// Selected auto-call delay in minutes. Null when auto-call is disabled (CVar set to 0).
    /// </summary>
    public required int? AutoCallDelayMinutes { get; init; }

    /// <summary>
    /// Elapsed time since the auto-call timer started, in minutes.
    /// Null when auto-call is disabled.
    /// </summary>
    public required double? AutoCallElapsedMinutes { get; init; }

    /// <summary>
    /// Whether an auto-call threshold elapsed and an automatic shuttle request was attempted.
    /// This does not claim the request was accepted.
    /// </summary>
    public required bool AutoCallAttempted { get; init; }

    /// <summary>
    /// Whether the round-end request token remains present.
    /// This can remain true after the shuttle countdown has elapsed.
    /// </summary>
    public required bool RoundEndRequested { get; init; }

    /// <summary>
    /// Remaining seconds until the shuttle countdown completes.
    /// Negative values indicate the countdown is overdue.
    /// Null when no countdown deadline is recorded.
    /// </summary>
    public required double? CountdownRemainingSeconds { get; init; }

    /// <summary>
    /// Whether the emergency shuttle has arrived at the station.
    /// </summary>
    public required bool EmergencyShuttleArrived { get; init; }

    /// <summary>
    /// Whether the emergency shuttles have left for CentCom.
    /// </summary>
    public required bool ShuttlesLeft { get; init; }

    /// <summary>
    /// Whether the round-scoped cancellation token for the eventual
    /// post-shuttle-departure EndRound callback remains armed.
    /// The token exists from round start; this does not claim the callback timer is already active.
    /// </summary>
    public required bool RoundEndCompletionTokenArmed { get; init; }
}
