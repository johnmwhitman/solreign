...
public sealed partial class AntagSelectionSystem
{
        ...
        AntagSpecifierPrototype def)
    {
        ...
        // SOLREIGN: skip accounts that drew antag in the last few rounds so the same handful of
        // regulars do not headline every shift. The gate caps its own exclusion set against the
        // live player count, so it can only ever REORDER who draws antag - it can never shrink the
        // pool to nothing and leave a round with no antagonist. Disable with
        // solreign.antag_rotation_enabled false.
        if (EntityManager.System<Content.Server._Solreign.Antags.SolreignAntagRotationGate>()
                .IsOnCooldown(player))
            return false;
