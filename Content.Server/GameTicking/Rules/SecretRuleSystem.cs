        ...
        while (options.Count > 0)
        {
            ...
            foreach (var (key, weight) in options)
            {
                ...
                if (!ProtoMan.TryIndex(key, out selectedPreset))
                    // Note: logs `key` (the weight-table entry that was rolled), not `selectedPreset`
                    // (always null here since TryIndex just failed) — a prior version of this log
                    // logged the null value, giving admins no way to tell which entry was broken.
            ...
            else if (selectedKey != string.Empty)
                Log.Info($"Excluding invalid preset '{selectedKey}' from secret preset selection.");
        }
