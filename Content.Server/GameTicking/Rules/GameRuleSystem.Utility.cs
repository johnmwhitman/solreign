...
public abstract partial class GameRuleSystem<T> where T: IComponent
{
        ...
        int numAttempts = 10)
    {
        ...
        for (var i = 0; i < numAttempts; i++)
        {
            ...
            // The rewrite that introduced the weighted-by-filled-tiles selection above (#44382)
            // never copied the chosen tile into the `tile` out-param: it stayed `default` (0,0)
            // for the whole call, so every "random" placement (station trash, puddle mess, and
            // every _Solreign rule that calls this - spore drift, acid storm, compliance hunter,
            // auditor prime, specimen zero) silently collapsed onto grid-local tile (0,0),
            // producing a single pile instead of a station-wide scatter. Assign it here so the
            // air-block check below and the final GridTileToLocal both use the tile that was
            // actually picked.
