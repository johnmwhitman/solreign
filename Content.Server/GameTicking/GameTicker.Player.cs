    ...
    public sealed partial class GameTicker
    {
        ...
        private async void PlayerStatusChanged(object? sender, SessionStatusEventArgs args)
        {
            ...
            async void AddPlayerToDb(Guid id)
            {
                ...
                {
                    // This is the exact "friend joins after the round already started" scenario: a
                    // brand-new session reaching SessionStatus.Connected mid-round, before the connection
                    // handshake's own UpdatePlayerRecordAsync write is guaranteed to have landed. This
                    // method is `async void` (it's the body of a SessionStatus.Connected event reaction,
                    // which can't itself be awaited), so any exception that escapes this await would go
                    // unobserved and crash the whole server -- that's exactly what used to happen here
                    // (KeyNotFoundException, see SolreignMidRoundConnectDbRaceTest). Log failures instead
                    // of letting them escape uncaught, so a DB hiccup here can never take the round down.
                    try
                    {
                        await _db.AddRoundPlayers(
                            RoundId,
                            new NetUserId(id),
                            session.Name,
                            session.Channel.RemoteEndPoint.Address,
                            session.Channel.UserData.GetModernHwid());
                    }
                    catch (Exception ex)
                    {
                        Log.Error($"Failed to record round-player DB association for {session}: {ex}");
                    }
                }
            }
        }
        ...
        public void PlayerJoinGame(ICommonSession session, bool silent = false)
        {
            ...
            FireAndLogAddRoundPlayers(session);
        ...
        /// <summary>
        /// Fires off <see cref="IServerDbManager"/>'s <c>AddRoundPlayers</c> for <paramref name="session"/> without
        /// blocking the (synchronous) caller, logging -- rather than silently discarding, or letting it
        /// escape unobserved and crash the server -- any failure. Neither <see cref="PlayerJoinGame"/> nor
        /// <see cref="PlayerJoinLobby"/> can be made `async Task` without rippling that change out to their
        /// several unrelated non-async callers, so this is the minimal-blast-radius way to stop discarding
        /// the returned task outright.
        /// </summary>
        private void FireAndLogAddRoundPlayers(ICommonSession session)
        {
            var task = _db.AddRoundPlayers(
                RoundId,
                session.UserId,
                session.Name,
                session.Channel.RemoteEndPoint.Address,
            ...
            LogOnFault(task, session);

            async void LogOnFault(System.Threading.Tasks.Task t, ICommonSession forSession)
            {
                try
                {
                    await t;
                ...
                {
                    Log.Error($"Failed to record round-player DB association for {forSession}: {ex}");
                }
            }
        }
