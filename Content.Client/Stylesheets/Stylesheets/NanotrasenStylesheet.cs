...
public partial class NanotrasenStylesheet : CommonStylesheet
{
    ...
    {
        // RobotoMono ships Regular/Bold/Italic and NO BoldItalic, so declare that honestly and let
        // GetFontPaths substitute Bold where BoldItalic is asked for. Claiming the full set made the
        // client throw "Content file does not exist for font" before any test could run.
        BaseFont = new NotoFontFamilyStack(
            ResCache,
            "/Fonts/RobotoMono/RobotoMono",
            "/Fonts/NotoSans/NotoSansSymbols",
            [FontKind.Regular, FontKind.Bold, FontKind.Italic]);
        var rules = new[]
        {
