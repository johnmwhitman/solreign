...
public sealed partial class EscapeUIController : UIController, IOnStateEntered<GameplayState>, IOnStateExited<GameplayState>
{
    ...
    public void OnStateEntered(GameplayState state)
    {
        ...
        // Solreign: roadmap D2.1 HUD/menu entry point -- reuses the existing "feedback" console
        // command (Content.Client/_Solreign/Feedback/FeedbackCommand.cs) rather than adding a new
        // EntitySystemManager dependency here, same idiom as the Disconnect/Quit buttons below.
        _escapeWindow.SolreignFeedbackButton.OnPressed += _ =>
        {
            CloseEscapeWindow();
            _console.ExecuteCommand("feedback");
        };

        // Solreign: fast "report a player" quick-action -- same idiom as the Feedback button above,
        // reuses the existing "report" console command (Content.Client/_Solreign/Report/ReportCommand.cs)
        // rather than adding a new EntitySystemManager dependency here.
        _escapeWindow.SolreignReportButton.OnPressed += _ =>
        {
            CloseEscapeWindow();
            _console.ExecuteCommand("report");
        };
