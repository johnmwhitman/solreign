...
public sealed partial class TestPair : RobustIntegrationTest.TestPair
{
    ...
    protected override RobustIntegrationTest.ServerIntegrationOptions ServerOptions()
    {
        ...
        // Every pooled server instance gets its OWN Season Ledger SQLite file. The ledger is a
        // persistent cross-round store whose round-envelope replay protection is keyed by round id,
        // and every pooled instance numbers its rounds from 1 — so the production path (a single
        // shared file in bin/ that also survives between runs) makes any two round-ending tests
        // collide with a replay-conflict StorageFailure. Unique temp path per instance = the same
        // discipline these tests already apply to player GUIDs.
        opts.CVarOverrides[CCVars.SolreignSeasonLedgerDbPath.Name] =
            Path.Combine(Path.GetTempPath(), $"solreign-test-ledger-{Guid.NewGuid():N}.db");
