...
public sealed class HumanoidProfileTests : GameTest
{
    ...
    public async Task EnsureValidRandomSpecies(string species)
    {
        ...
        await Server.WaitAssertion(() =>
        {
            LoadDependencies(out body, out var humanoidComponent);
        ...
        await Server.WaitAssertion(() =>
        {
            Assert.That(SEntMan.EntityExists(body), Is.True, $"{species} did not survive the soak ticks.");
            Assert.That(SEntMan.GetComponent<HumanoidProfileComponent>(body).Species, Is.EqualTo(species), $"Species changed during soak ticks. Expected: {species}");
            Assert.That(SEntMan.System<DamageableSystem>().GetTotalDamage((body, SEntMan.GetComponent<DamageableComponent>(body))), Is.EqualTo(FixedPoint2.Zero), $"{species} accumulated damage during soak ticks.");
        });
    }
