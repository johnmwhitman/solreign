...
namespace Content.IntegrationTests.Tests.GameRules
{
    ...
    public sealed class RuleMaxTimeRestartTest : GameTest
    {
        ...
        public async Task RestartTest()
        {
            ...
            {
                sGameTicker.StartGameRule(MaxTimeRestartGameRule, out ruleEntity);
            ...
            // Solreign auto-starts the SolreignStationDirective and SolreignCorporate game rules
            // on every round start, so count only the rule under test instead of all game rules.
            Assert.That(server.EntMan.Count<MaxTimeRestartRuleComponent>(), Is.EqualTo(1));
            Assert.That(server.EntMan.HasComponent<ActiveGameRuleComponent>(ruleEntity));
